...
//  buoi3
//
//  Created by TuanAnh on 27/11/24.
//
        ...
        GeometryReader { geo in
            ScrollViewReader{ proxy in
                ...
                ZStack(alignment: .top) {
                    Image(show.removeCaseAndSpace()).resizable().scaledToFit()
                    ScrollView {
                        TabView {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            Text("Portrayed By :\(character.portrayedBy)").font(.subheadline)
                            Divider()
                                ...
                                VStack(alignment: .leading) {
                                    ...
                                    if let death = character.death {
                                        AsyncImage(url: death.image) { image in
                                            image.resizable().scaledToFit().clipShape(.rect(cornerRadius: 15)).onAppear {
                                                withAnimation {
