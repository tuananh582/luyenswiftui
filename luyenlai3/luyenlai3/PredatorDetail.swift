        ...
        GeometryReader{ geo in
            ...
            ScrollView {
                ZStack(alignment: .bottomTrailing){
                    ...
                    //Dino Image
                    Image(predator.image).resizable().scaledToFit().frame(width: geo.size.width/1.5,height: geo.size.height/3.4)
                        .scaleEffect(x:-1).shadow(color: .black, radius: 11).offset(y:20)
                ...
                VStack(alignment:.leading){
                    ...
                    NavigationLink{
                        ...
                        Map(position:$position){
                            Annotation(predator.name, coordinate: predator.location) {
                                Image(systemName: "mappin.and.ellipse").foregroundStyle(.blue).symbolEffect(.pulse).imageScale(.large).font(.largeTitle)
                            }
                        ...
                        .overlay(alignment:.topLeading) {
                            Text("Current Location").padding([.leading,.bottom],5).padding(.trailing,8).background(.black.opacity(0.33)).clipShape(.rect(bottomLeadingRadius: 15))
                        }
                    }
                    ...
                    ForEach(predator.movieScenes) { scence in
                        Text(scence.movie).font(.title2).padding(.vertical,1)
                        Text(scence.sceneDescription).padding(.bottom,15)
                    }
